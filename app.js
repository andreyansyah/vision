document.addEventListener('DOMContentLoaded', () => {
    ...
    if (taskForm) {
        taskForm.addEventListener('submit', (e) => {
            ...
            if (projectsList) {
                ...
                const variations = [
                    { img: 'projects/xyora.svg', class: 'project-xyora' },
                    { img: 'projects/computask.png', class: 'project-computask' },
                    { img: 'projects/mkt-central-directory.png', class: 'project-ctigroup' }
                ];
    ...
    if (deviceGemini) {
        ...
        const projectLogos = {
            'Xyora': 'projects/xyora.svg',
            'Computask': 'projects/computask.png',
            'MKT Central Directory': 'projects/mkt-central-directory.png'
        };
        ...
        function updateActiveProject(name) {
            ...
            if (sidebarLogo) {
                sidebarLogo.src = projectLogos[projectName] || 'projects/xyora.svg';
        ...
        function showBotTypingAndReply(userQuery) {
            ...
            if (queryLower.includes('halo') || queryLower.includes('hi') || queryLower.includes('pagi') || queryLower.includes('siang') || queryLower.includes('sore') || queryLower.includes('malam') || queryLower.includes('hei') || queryLower.includes('hello')) {
                answer = `Halo! Saya adalah asisten pintar Anda untuk proyek **${projectName}**. Sebaiknya kita mulai dari mana untuk memajukan proyek ini? Anda bisa menanyakan target tenggat waktu, pembagian tugas, atau deskripsi teknis proyek.`;
            } else if (queryLower.includes('fitur') || queryLower.includes('apa saja') || queryLower.includes('tentang') || queryLower.includes('deskripsi') || queryLower.includes('buat apa') || queryLower.includes('tujuan')) {
                if (projectName.toLowerCase() === 'xyora') {
                    answer = `Proyek **Xyora** adalah platform manajemen tugas kolaboratif premium. Fitur utamanya mencakup visualisasi timeline, swipe-to-delete pada daftar note, serta dashboard manajemen terpusat dengan sub-sheet drawer yang interaktif.`;
                } else if (projectName.toLowerCase() === 'computask') {
                    answer = `Proyek **Computask** difokuskan pada otomatisasi alur kerja digital. Fitur utamanya mencakup integrasi pemicu tugas otomatis, pelacakan waktu nyata, dan sinkronisasi berkas antarmuka pengguna secara mulus.`;
                } else {
                    answer = `Proyek **${projectName}** difokuskan pada optimalisasi kinerja tim dan visualisasi pencapaian tugas secara berkala.`;
                }
            } else if (queryLower.includes('tugas') || queryLower.includes('task') || queryLower.includes('kerja') || queryLower.includes('milestone')) {
                answer = `Berdasarkan data hari ini, seluruh milestone tugas untuk **${projectName}** berjalan lancar. Anda dapat menambahkan tugas baru di dashboard atau menjadwalkan tenggat waktu di tab Calendar/Schedule. Ada bagian khusus yang ingin Anda selesaikan hari ini?`;
            } else {
                answer = `Terima kasih! Pertanyaan Anda tentang "${userQuery}" sangat menarik untuk pengembangan proyek **${projectName}**. Saya menyarankan kita memfokuskan alur kerja pada bagian integrasi antarmuka agar visualisasinya tetap elegan dan interaktif. Apakah ada detail spesifik lain yang ingin Anda bahas?`;
            }
            
            setTimeout(() => {
                const textContainer = botContainer.querySelector('.gemini-bot-text');
                ...
                function typeWriter() {
                    if (i < answer.length) {
                        const currentText = answer.substring(0, i + 1);
                        const parsedHtml = currentText.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
                        textContainer.innerHTML = parsedHtml;
                        i++;
                        scrollToBottom();
                        setTimeout(typeWriter, speed);
                    } else {
                        textContainer.innerHTML = answer.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
                        scrollToBottom();
                    }
                }
                typeWriter();
